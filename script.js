...
fileInput.addEventListener("change", async () => {
  ...
  reader.onload = async function (e) {
    ...
    // Call detection with base64 or upload logic
    const predictionResult = await prediction(e.target.result); // you already wrote prediction()
    let carCount = 0;
    predictionResult.predictions.forEach(p => {
      if (p.class === "car") carCount++;
    });

    resultsDiv.innerHTML = `<strong>Detected cars:</strong> ${carCount}`;
  };
  reader.readAsDataURL(file); // converts to base64 so fetch works
});
